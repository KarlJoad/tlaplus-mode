...
.PHONY: lisp docs \
	...
	test test-interactive tla+-mode \
	clean clean-lisp clean-docs clean-archives \
	stats bump-versions bump-snapshots \
	dist versionlib tla+-mode-$(VERSION).tar.gz
...
help:
	...
	$(info make emacs-Q          - run emacs -Q plus tla+-mode)
...
clean-archives:
	@$(RM) *.tar.gz *.tar lisp/tla+-mode-version.el
	@$(RMDIR) tla+-mode-$(VERSION)
...
dist: tla+-mode-$(VERSION).tar.gz

versionlib:
	@$(MAKE) -C lisp versionlib

DIST_ROOT_FILES = LICENSE default.mk Makefile README.md
DIST_LISP_FILES = $(addprefix lisp/,$(ELS) tla+-mode-version.el Makefile)
...
tla+-mode-$(VERSION).tar.gz: lisp versionlib info
	@printf "Packing $@\n"
	@$(MKDIR) tla+-mode-$(VERSION)
	@$(CP) $(DIST_ROOT_FILES) tla+-mode-$(VERSION)
	@$(MKDIR) tla+-mode-$(VERSION)/lisp
	@$(CP) $(DIST_LISP_FILES) tla+-mode-$(VERSION)/lisp
	@$(MKDIR) tla+-mode-$(VERSION)/docs
	@$(CP) $(DIST_DOCS_FILES) tla+-mode-$(VERSION)/docs
	@$(TAR) cz --mtime=./tla+-mode-$(VERSION) -f tla+-mode-$(VERSION).tar.gz tla+-mode-$(VERSION)
	@$(RMDIR) tla+-mode-$(VERSION)
